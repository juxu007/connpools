...
// TCtxKey: context key type
type TCtxKey string

// InitParams: connection pools initialization params
type InitParams struct {
	ZkTimeout     time.Duration
	SocketTimeout time.Duration

	PoolSize          int32
	PoolFuseSecond    int64
	PoolFailTimesSucc float64

	FuseThreshold     int64
	NodeFuseSecond    int64
	NodeFailTimesSucc float64

	MaxWeight       int32
	AddedWeight     int32
	FluctuateWeight int32
}

var (
	// context key
	RequestID     TCtxKey = "request_id"
	SocketTimeout TCtxKey = "socket_timeout"

	// connection pools configuration
	poolSize int32 = 64

	// zookeeper timeout configuration
	zkTimeout = 90 * time.Second
...
// func init() {
// 	logs.SetLogger(logs.AdapterFile, `{"filename":"conn_pool_logs/conn_pool.log","level":7,"maxlines":1000000,"daily":true,"maxdays":2,"color":true, "perm": "0765", "rotateperm": "444"}`)
...
func Init(zkAddrs string, services map[string]string, optionalInitParams *InitParams) {
	if optionalInitParams != nil {
		if optionalInitParams.ZkTimeout <= time.Duration(0) {
			logs.Warn("the timeout value of connection to zookeeper is setted to default: %v", zkTimeout)
		} else {
			zkTimeout = optionalInitParams.ZkTimeout
		}
		if optionalInitParams.SocketTimeout <= time.Duration(0) {
			logs.Warn("the default socket timeout value of connection to servers is setted to default: %v", socketTimeout)
		} else {
			socketTimeout = optionalInitParams.SocketTimeout
		}
		if optionalInitParams.PoolSize <= 0 {
			logs.Error("the connection pool size is setted to default: %v", poolSize)
		} else {
			poolSize = optionalInitParams.PoolSize
		}
		if optionalInitParams.PoolFuseSecond <= 0 {
			logs.Warn("the fuse duration of pools is setted to default: %v", poolFuseSecond)
		} else {
			poolFuseSecond = optionalInitParams.PoolFuseSecond
		}
		if optionalInitParams.PoolFailTimesSucc <= 0 {
			logs.Error("the multiple of pool's failure times success is setted to default: %v", poolFailTimesSucc)
		} else {
			poolFailTimesSucc = optionalInitParams.PoolFailTimesSucc
		}
		if optionalInitParams.FuseThreshold <= 0 {
			logs.Warn("the minimum of failure per node in fuse window is setted to default: %v", fuseThreshold)
		} else {
			fuseThreshold = optionalInitParams.FuseThreshold
		}
		if optionalInitParams.NodeFuseSecond <= 0 {
			logs.Warn("the fuse duration of node is setted to default: %v", nodeFuseSecond)
		} else {
			nodeFuseSecond = optionalInitParams.NodeFuseSecond
		}
		if optionalInitParams.NodeFailTimesSucc <= 0 {
			logs.Error("the multiple of node's failure times success is setted to default: %v", nodeFailTimesSucc)
		} else {
			nodeFailTimesSucc = optionalInitParams.NodeFailTimesSucc
		}
		if optionalInitParams.MaxWeight <= 0 {
			logs.Error("the maximum of node's weight is setted to default: %v", maxWeight)
		} else {
			maxWeight = optionalInitParams.MaxWeight
		}
		if optionalInitParams.AddedWeight <= 0 {
			logs.Error("the increase of node's weight is setted to default: %v", addedWeight)
		} else {
			addedWeight = optionalInitParams.AddedWeight
		}
		if optionalInitParams.FluctuateWeight <= 0 {
			logs.Warn("the Fluctuation of node's weight is setted to default: %v", fluctuateWeight)
		} else {
			fluctuateWeight = optionalInitParams.FluctuateWeight
		}
	} else {
		logs.Info("use default setting params")
	}
	...
	go func() {
		...
		for {
			if status == statusConnecting {
				...
				if err == nil {
					syncChildren(serviceName, path, children)
					status = statusWatch
...
func ConnPoolDel(serviceName string) {
	...
	if found {
		...
		logs.Info("pool deleted: %s->%v", serviceName, pool.value)
	} else {
		logs.Error("pool %v not found", serviceName)
	}
}
...
func RemoteCall(ctx context.Context, serviceName string, clientCallback, funcCallback interface{}, args ...interface{}) (interface{}, error) {
	...
	if ctx != nil {
		...
		if ctx.Value(SocketTimeout) != nil {
			t, ok := ctx.Value(SocketTimeout).(time.Duration)
			if ok {
				timeout = t
			} else {
	...
	callThriftFunc := func() error {
		...
		if len(resv) == 0 {
			...
			err := errors.New("resp is nil: context timeout exceeded")
	...
	for loop {
		...
		case conn, ok = <-node.poolChan:
			...
			// connCheck: This is not needed
			// // if connCheck(conn.socket.Conn()) != nil {
			// // 	if err := conn.close(); err != nil {
			// // 		logs.Error(err)
			// // 	}
			// // 	// logs.Info("%v|%v|%v|conn is broken, trying next", requestID, serviceName, conn.address)
			// // 	continue
			// // }
	...
	for i, v := range resv {
		if i == len(resv)-1 {
			if v.Interface() != nil {
				logs.Error("this is fatal error")
...
func RemoteCallInParallel(ctx context.Context, serviceName string, clientCallback, funcCallback, arg interface{}, goNum int) ([]interface{}, error) {
	...
	if reflect.TypeOf(arg).Kind() == reflect.Slice {
		...
		return nil, errors.New("args is not kind of reflect.Slice")
	}
