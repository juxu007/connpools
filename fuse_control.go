...
var (
	fuseThreshold = int64(2)

	poolFuseSecond    = int64(5)
	poolFailTimesSucc = float64(2)

	nodeFuseSecond    = int64(5)
...
func (f *fuseControl) onReject() {
	...
	if f.nodeName == "" && f.totalFail > poolFailTimesSucc*f.totalSucc {
		if p, loaded := pools.Load(f.poolName); loaded {
			if pool, ok := p.(*pool); ok {
				pool.RLock()
				pLen := len(pool.nodes)
				pool.RUnlock()
				if f.totalFail > float64(int64(pLen)*fuseThreshold) {
					f.fusing()
				}
			} else {
		...
		if f.totalFail > float64(fuseThreshold) {
			f.fusing()
		}
	}
}
...
func (f *fuseControl) available() bool {
	if atomic.LoadInt32(&f.fuse) == 1 {
		...
		if f.nodeName == "" && second > lastRecordSecond && second-lastRecordSecond > poolFuseSecond {
			atomic.StoreInt32(&f.fuse, 0)
			logs.Info("pool %v is recover from fusing", f.poolName)
			return true
		} else if f.nodeName != "" && second > lastRecordSecond && second-lastRecordSecond > nodeFuseSecond {
			atomic.StoreInt32(&f.fuse, 0)
			logs.Info("node %v in pool %v is recover from fusing", f.nodeName, f.poolName)
			return true
		}
		return false
	} else {
