...
func newThriftClient(address string, timeout time.Duration) (*thriftClient, error) {
	...
	socket, err := thrift.NewTSocketConf(address, tConfig)
	if err != nil {
		return nil, err
	}

	//protocol
	protocolFactory := thrift.NewTBinaryProtocolFactoryConf(tConfig)

	//transportFactory
	transportFactory := thrift.NewTFramedTransportFactoryConf(thrift.NewTTransportFactory(), tConfig)
